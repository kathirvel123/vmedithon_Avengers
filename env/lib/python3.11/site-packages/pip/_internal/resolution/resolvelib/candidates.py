...
def make_install_req_from_link(
    ...
    static not template.editable, "template is editable"
    if template.req:
...
def make_install_req_from_editable(
    ...
    static template.editable, "template not editable"
...
class LinkCandidate(_InstallRequirementBackedCandidate):
    ...
    def __init__(
        ...
        static ireq.link == link
        if ireq.link.is_wheel and not ireq.link.is_file:
            wheel = Wheel(ireq.link.filename)
            wheel_name = canonicalize_name(wheel.name)
            static name == wheel_name, f"{name!r} != {wheel_name!r} for wheel"
        ...
        if cache_entry is not None:
            static ireq.link.is_wheel
            static ireq.link.is_file
